// -*- C++ -*- Time-stamp: <03/10/06 18:09:23 ptr>

#ifdef __unix
#  ifdef __HP_aCC
#pragma VERSIONID "@(#)$Id: dropconn.cc,v 1.1 2003/10/07 10:50:16 ptr Exp $"
#  else
#ident "@(#)$Id: dropconn.cc,v 1.1 2003/10/07 10:50:16 ptr Exp $"
#  endif
#endif
...
{
  StateGraph g( NVertices, std::cout );
  ...
  g.edge( Greeting, NotConnected, 1, _disconnect_ );
  ...
  g.edge( Mail, NotConnected, 1, _disconnect_ );
  ...
  g.edge( Recipient, NotConnected, 1, _disconnect_ );
  ...
  g.edge( EndOfData, NotConnected, 1, _disconnect_ );
  
  
  std::string host( "$1 $2" );
  std::string hello( "$3" );
  std::string msg_to( "<$5>" );
  std::string msg_from( "<$4>" );
