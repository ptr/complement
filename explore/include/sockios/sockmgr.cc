// -*- C++ -*- Time-stamp: <09/02/18 15:29:45 ptr>
    ...
    if ( ev.events & EPOLLIN ) {
      ...
      for ( ; ; ) {
        ...
        if ( offset < 0 ) {
          ...
          //      << std::system_error( errno, std::get_posix_category(), std::string( "sockmgr<charT,traits,_Alloc>::push( socks_processor_t& p )" ) ).what() << endl;
          throw fdclose();
        }
