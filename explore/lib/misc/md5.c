...
void MD5Update( MD5_CTX *context, const uint8_t* input, size_t inputLen )
{
  ...
  */
  if ( inputLen >= partLen ) {
    MD5_memcpy( context->buf_un.buf8 + index, input, partLen );
    MD5Transform( context->state, context->buf_un.buf8 );
  ...
  MD5_memcpy( context->buf_un.buf8 + index, input + i, inputLen - i );
}
 ...
 */
#if 0
static void MD5_memcpy( char *output, char *input, unsigned len )
{
  unsigned i;
 ...
 */
static void MD5_memset( char *output, int value, unsigned len )
{
  unsigned i;
